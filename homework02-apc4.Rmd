...
So far this course has taught me a lot about the usefulness and vast applications of R and data management. Coming into this course I had no background in R or statistics, other than STAT400, but was very surprised at how efficient the teaching method of driver-based labs and homework assignments have been in bridging the gap from not knowing any R to be able to effectively contribute and complete assignments. I am very excited from the material we have covered so far as to the range of possibilities that R and data management has inside and outside of this course.
...
1. September shows the greatest amounts of new cases amongst the university community
2. The case positivity rate has maintained under 2% since September 2020
3. The months of March and April 2021 when vaccines were becoming more available show that total cases was much lower than most months

**Questions**

1. What is more important when evaluating campus safety, total cases or case positivity?
2. What was the reason for the spike in case positivity in July 2021?
3. What is the current outlook on this data with August and September 2021?
...
library(rjson)
congress = fromJSON(file = "https://theunitedstates.io/congress-legislators/legislators-current.json")
str(congress)
...
rental_inspections_grades_data01 <- read_csv("https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/rental-inspections-grades-data01.csv?token=AAADBLGDV5S454SOBCOLRDLBLNYXM")
...
data = read_csv("https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/rental-inspections-grades-data02.txt?token=AAADBLHWCRUUKDWTMLAZUKDBLO3A2")

data$InspectionDate2 <-lubridate::mdy(data$`Inspection Date`)
data$ExpirationDate2<-lubridate::mdy(data$`Expiration Date`)
mode(data$InspectionDate2)
mode(data$ExpirationDate2)
typeof(data$InspectionDate2)
typeof(data$ExpirationDate2)
str(data)

data %>% 
  arrange(desc(InspectionDate2)) %>%
  head(., 10)
