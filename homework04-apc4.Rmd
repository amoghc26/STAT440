...
str(ads)
trips = read_delim("https://uofi.box.com/shared/static/0pyqnfpl40kr5do7fue8t85vva11ynjm.txt", delim = "\t")
str(trips)
store = read_delim("https://uofi.box.com/shared/static/o2bqah52ioj0p0q4msvna7ixk8d9lsh4", delim = " ")
str(store)
...
big_spender = trips %>%
  ...
  select(-c(WEEK)) %>%
  group_by(PANID) %>%
  summarise(Total_Dollars = sum(dollars999)) %>%
  filter(Total_Dollars > 15000)
...
```{r}
colnames(trips)[3] = "IRI_KEY"
combine = inner_join(store, trips, by = "IRI_KEY") 
combine = inner_join(combine, big_spender, by = "PANID")

combine2 = combine %>%
  select(c(Market_Name, PANID, Total_Dollars)) %>%
  group_by(Market_Name) %>%
  tally()

combine2
```
Panelists in 'big_spender' shop in Pittsfield. 
...
```{r}
library(tidyverse)
store2 = sort(unique(store$Market_Name))
store2
```
...
```{r}
library(tidyverse)
name = toupper(c("Atlanta", "Birmingham/Montg.", "Charlotte", "Dallas, TX", "Raleigh/Durham", "Durham", "Houston", "Knoxville", "Mississippi", "Montgomery", "New Orleans, LA", "Norfolk", "Raleigh", "Richmond/Norfolk", "South Carolina" ))

southern_grocery = store %>%
  filter(Market_Name == name) %>%
  mutate(single_state_market = ifelse(Market_Name == c("MISSISSIPPI", "SOUTH CAROLINA"), "YES", "NO")) 
southern_grocery$Market_Name = gsub("(.*),.*", "\\1",southern_grocery$Market_Name)
southern_grocery$Market_Name = gsub("/", " & ",southern_grocery$Market_Name)

southern_grocery = southern_grocery %>%
  arrange(Market_Name)

head(southern_grocery, n=5)
tail(southern_grocery, n=5)
```
...
panel = ads %>%
  filter(Children.Group.Code == 7) 
colnames(panel)[1] = "PANID"

colnames(trips)[3] = "IRI_KEY"
panel2 = inner_join(store, trips, by = "IRI_KEY") %>%
  filter(WEEK <= min(WEEK)+40 ) %>%
  group_by(Market_Name, PANID) %>%
  summarise(total = sum(CENTS999/100)) %>%
  filter(total >=100) 

panel3 = inner_join(panel2, panel, by = "PANID" ) %>%
  select(c(Market_Name, PANID, total, Family.Size, Children.Group.Code)) %>%
  tally()
...
There are 0 panelists with at least one child aged 0-17 years, that spent at least one hundred dollars at each of at least 3 different grocery stores within the first 40 weeks of the year.
