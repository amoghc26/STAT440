...
```{r}
library(readr)
food = read_csv("https://uofi.box.com/shared/static/lwwzlv2sulip1p8qoqqghmw0pouso0wk.csv")
head(food, n=10)
```
...
library(lubridate)
jail2 = jail 
jail2$booking_date <- as.Date(jail$booking_date, format="%m/%d/%y")
jail2$released_date <- as.Date(jail$released_date, format="%m/%d/%y")
jail2$released_time <- strptime(jail$released_time, format="%H:%M:%S")
jail2$booking_time <- strptime(jail$booking_time, format="%H:%M:%OS")
str(jail2$released_time)
```

```{r}
library(tidyverse)
library(readr)
food0 = read_csv("https://uofi.box.com/shared/static/lwwzlv2sulip1p8qoqqghmw0pouso0wk.csv", col_types = cols(`Inspection Date` = col_date(format = "%m/%d/%Y")))
...
library(plyr)
food2 = food %>%
  mutate(`Inspection Date`=lubridate::mdy(`Inspection Date`)) %>%
  filter(`Inspection Date`>09/01/2011) %>%
  mutate(`pass_or_not`=ifelse(`Results`=="Pass" | `Results`=="Pass w/ Conditions",1,0)) %>%
  select(-c("Address", "Census Tracts", "City", "Community Areas", "Historical Wards 2003-2015", "Location", "State", "Wards","Zip Codes")) %>%
  na.omit(food)
...
food4 = setDT(count(food3$`Facility Type`)) %>%
  arrange(desc(`freq`))
...
**Schools are the third most common facility type to fail.**
...
library(tidyverse)
jail0 = jail%>%
  mutate(`booking_date_time` = paste0(jail$booking_date, " ", jail$booking_time, "")) %>%
  mutate(`booking_date_time` = as.POSIXlt.character(`booking_date_time`, format = "%m/%d/%Y %H:%M:%S")) %>%
  mutate(`released_date_time` = paste0(jail$released_date, " ", jail$released_time, "")) %>%
  mutate(`released_date_time` = as.POSIXlt.character(`released_date_time`, format = "%m/%d/%Y %H:%M:%S"))

jail0 = jail0 %>% 
  mutate(`years` = (lubridate::year(jail0$released_date_time)) - (lubridate::year(jail0$booking_date_time))) %>%
  mutate(`days` = (lubridate::day(jail0$released_date_time)) - (lubridate::day(jail0$booking_date_time))) %>%
  mutate(`hours` = (lubridate::hour(jail0$released_date_time)) - (lubridate::hour(jail0$booking_date_time))) %>%
  mutate(`minutes` = (lubridate::minute(jail0$released_date_time)) - (lubridate::minute(jail0$booking_date_time))) %>%
  mutate(`seconds` = (lubridate::second(jail0$released_date_time)) - (lubridate::second(jail0$booking_date_time))) %>%
  mutate(`time_spent_in_jail1` = (years*365*24)+(days*24)+hours+(minutes*(1/60))+(seconds*(1/3600))) %>%
  mutate(`time_spent_in_jail2` = as.double(jail0$released_date_time - jail0$booking_date_time, units="hours") ) 
jail0

inmates = jail0 
names(inmates)[names(inmates)=="booking_number"] = "booked_number"
names(inmates)[names(inmates)=="booking_date"] = "booked_date"
names(inmates)[names(inmates)=="booking_time"] = "booked_time"
names(inmates)[names(inmates)=="booking_reason"] = "booked_reason"
names(inmates)[names(inmates)=="booking_date_time"] = "booked_date_time"


head(inmates[, c("jacket_number", "time_spent_in_jail1", "time_spent_in_jail2")],10)
```
